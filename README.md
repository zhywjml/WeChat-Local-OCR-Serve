...
极致轻量、隐私安全、低延迟的本地 OCR 解决方案。A lightweight, privacy-focused, low-latency local OCR solution based on WeChat's OCR engine.
...
2. Obsidian 全库图片搜索插件 (Obsidian Global Search)
   **状态**：✅ 已发布 | **点击前往插件仓库**:[ obsidian-local-wechatocr-search](https://github.com/zhywjml/obsidian-local-wechatocr-search)
   - 专为知识库打造的**视觉搜索引擎**。
   - **全库检索**：建立本地索引，检索库内所有图片中的文字。
   - **可视定位**：搜索结果提供左右分栏预览，使用**蓝色呼吸框**精准高亮文字位置。
   - **溯源跳转**：自动列出引用该图片的笔记，点击即可**精准跳转**到图片所在的具体行。
...
1.  在 Releases 页面下载最新版安装包 `WeChatOCR_Setup_v1.7.exe`。
2.  运行安装程序，建议勾选“创建桌面快捷方式”和“开机自动启动”。
3.  安装完成后，服务会自动运行（查看任务栏右下角蓝色图标）。

### 方式二：源码运行 (开发者)

1.  克隆本项目。
2.  确保根目录下包含 `wxocr` 文件夹（需自行提取或下载 Release 包获取）。
3.  安装依赖：
    ```bash
...
- **[wechat-ocr](https://github.com/swigger/wechat-ocr)**：感谢 swigger 对微信 OCR 组件的分析与封装，本项目基于其核心逻辑进行了 HTTP 服务化封装。
